...
awful.keyboard.append_global_keybindings({
    ...
    awful.key({ modkey }, "p", function() awful.spawn(apps.launcher) end, { description = "run search", group = "launcher" }),
...
awful.keyboard.append_global_keybindings({
    ...
    awful.key({ modkey, }, "Tab", function() awesome.emit_signal("bling::window_switcher::turn_on") end, { description = "Следующее окно", group = "client" }),

    awful.key({ modkey, }, "b", function() lock_screen_show()    end,   {description = "Фокус на верхнее окно",             group = "client"}),
})

-- Слои из библиотеки machi
awful.keyboard.append_global_keybindings({
    awful.key({ modkey }, ".", function() layout_machi.default_editor.start_interactive() end, { description = "edit current layout", group = "layout" }),
    awful.key({ modkey, shiftkey }, ".", function() layout_machi.switcher.start(client.focus) end, { description = "switch between windows for a machi layout", group = "layout" }),
})
...
awful.keyboard.append_global_keybindings({
    awful.key({ modkey,           }, "j", function() awful.client.focus.bydirection("down") bling_flush(client.focus) end, {description = "Фокус на нижнее окно",              group = "client"}),
    awful.key({ modkey,           }, "h", function() awful.client.focus.bydirection("left")  bling_flush(client.focus) end,   {description = "Фокус на левое окно",               group = "client"}),
    awful.key({ modkey,           }, "l", function() awful.client.focus.bydirection("right") bling_flush(client.focus) end,   {description = "Фокус на правое окно",              group = "client"}),
    awful.key({ modkey,           }, "k", function() awful.client.focus.bydirection("up")    bling_flush(client.focus) end,   {description = "Фокус на верхнее окно",             group = "client"}),
    ...
    awful.key({ modkey, shiftkey  }, "j", function () move_client(client.focus, 'down') end,    {description = "Переместить окно вниз",        group = "client"}),
    awful.key({ modkey, shiftkey  }, "h", function () move_client(client.focus, 'left') end,    {description = "Переместить окно влево",       group = "client"}),
    awful.key({ modkey, shiftkey  }, "l", function () move_client(client.focus, 'right') end,   {description = "Переместить окно вправо",      group = "client"}),
    awful.key({ modkey, shiftkey  }, "k", function () move_client(client.focus, 'up') end,      {description = "Переместить окно вверх",       group = "client"}),
...
client.connect_signal("request::default_keybindings", function()
    awful.keyboard.append_client_keybindings({
        awful.key({ modkey,         }, "f", function (c) c.fullscreen = not c.fullscreen c:raise() end, {description = "Переключать полноэкранный режим окон",  group = "client"}),
        ...
        awful.key({ modkey,         }, "z",     function (c) c.maximized = not c.maximized c:raise() end , {description = "(un)maximize", group = "client"}),
